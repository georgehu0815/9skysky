...
Invoked via the `train` wrapper (see train_cli.py):
    ...
    uv run rlx-mjlab-train Mjlab-Kick-Flat-MicroDuck \
...
`uv run rlx-mjlab-train` inside the job.
...
pushed by a watcher running alongside training (rlx.mjlab_microduck.tools.hf.uploader) to a
...
mkdir -p logs/rsl_rl
nohup uv run python -m rlx.mjlab_microduck.tools.hf.uploader > /tmp/uploader.log 2>&1 &
UPLOADER_PID=$!

echo "[bootstrap] starting training: uv run rlx-mjlab-train $TRAIN_ARGS"
set +e
uv run rlx-mjlab-train $TRAIN_ARGS
TRAIN_RC=$?
set -e
...
CKPT_ONE_SHOT=1 uv run python -m rlx.mjlab_microduck.tools.hf.uploader || true
...
if [ "$TRAIN_RC" -eq 0 ] && [ "${AUTO_EXPORT:-1}" = "1" ]; then
    ...
    if [ -n "$CKPT" ]; then
        ...
        uv run rlx-mjlab-export "$TASK_ID" \
